...
class GCPExtractionRequest(BaseModel):
    ...
    credentials_json: Optional[Dict[str, Any]] = Field(
        None, description="Service account credentials as JSON"
    )
    include_global_resources: bool = Field(
        True, description="Include global resources (Cloud Storage, IAM, etc.)"
    )
    labels_filter: Optional[Dict[str, str]] = Field(
        None, description="Filter by labels"
    )
...
async def _perform_gcp_extraction(
    ...
    try:
        ...
        for region in regions_to_scan:
            logger.info(
                "Extracting resources from GCP region", region=region, job_id=job_id
            )
        ...
        logger.info(
            "Completed GCP resource extraction",
            job_id=job_id,
        ...
        logger.error(
            "GCP extraction job failed: %s", str(e), exc_info=True, job_id=job_id
        )
