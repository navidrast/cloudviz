...
class AWSExtractionRequest(BaseModel):
    ...
    include_global_resources: bool = Field(True, description="Include global resources (S3, IAM, etc.)")
...
async def extract_aws_resources(
    ...
    try:
        ...
        logger.info("Started AWS extraction job", job_id=job_id, 
                   account_id=request.account_id, user_id=current_user.get("id"))
...
async def _perform_aws_extraction(
    ...
    try:
        ...
        for region in regions_to_scan:
            logger.info("Extracting resources from AWS region", region=region, job_id=job_id)
        ...
        logger.info("Completed AWS resource extraction", 
                   job_id=job_id, total_resources=len(all_resources))
        
    except Exception as e:
        logger.error("AWS extraction job failed: %s", str(e), exc_info=True, job_id=job_id)
