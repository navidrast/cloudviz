...
async def rate_limiting_middleware(request: Request, call_next):
    ...
    if client_ip not in rate_limit_storage:
        rate_limit_storage[client_ip] = {"requests": [], "blocked_until": 0}
...
async def error_handling_middleware(request: Request, call_next):
    ...
    try:
        ...
        return JSONResponse(
            status_code=e.status_code, content=error_response.dict(), headers=e.headers
        )
        ...
        logger.error(
            "Unhandled exception occurred: %s [correlation_id=%s, path=%s, method=%s]",
            str(e),
            correlation_id,
            request.url.path,
            request.method,
