...
class ResourceInventory:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "extraction_scope": (
                self.extraction_scope.value if self.extraction_scope else None
            ),
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ResourceInventory":
        ...
        inventory = cls()
        inventory.resources = [
            CloudResource.from_dict(r) for r in data.get("resources", [])
        ]
        inventory.relationships = [
            ResourceRelationship.from_dict(r) for r in data.get("relationships", [])
        ]
        inventory.metadata = data.get("metadata", {})
        inventory.extraction_time = datetime.fromisoformat(data["extraction_time"])
        inventory.extraction_scope = (
            ExtractionScope(data["extraction_scope"])
            if data.get("extraction_scope")
            else None
        )
        inventory.scope_identifier = data.get("scope_identifier")
        inventory.provider = (
            CloudProvider(data["provider"]) if data.get("provider") else None
        )
        return inventory
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        for resource in self.resources:
            ...
            stats["resource_types"][type_name] = (
                stats["resource_types"].get(type_name, 0) + 1
            )

            # Count by provider
            provider_name = resource.provider.value
            stats["providers"][provider_name] = (
                stats["providers"].get(provider_name, 0) + 1
            )

            # Count by region
            stats["regions"][resource.region] = (
                stats["regions"].get(resource.region, 0) + 1
            )

            # Count by resource group
            if resource.resource_group:
                stats["resource_groups"][resource.resource_group] = (
                    stats["resource_groups"].get(resource.resource_group, 0) + 1
                )
