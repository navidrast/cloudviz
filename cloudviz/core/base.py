...
class EventHook(ABC):
    ...
    @abstractmethod
    async def on_extraction_completed(
        self, job: ExtractionJob, inventory: ResourceInventory
    ):
    ...
    @abstractmethod
    async def on_visualization_generated(
        self, inventory: ResourceInventory, output_format: str, result: bytes
    ):
...
class CacheManager(ABC):
    ...
    @abstractmethod
    async def get_cached_inventory(self, cache_key: str) -> Optional[ResourceInventory]:
        """
    ...
    async def cache_inventory(
        self, cache_key: str, inventory: ResourceInventory, ttl_seconds: int = 3600
    ) -> bool:
        """
    ...
    @abstractmethod
    async def get_cached_visualization(self, cache_key: str) -> Optional[bytes]:
        """
    ...
    async def cache_visualization(
        self, cache_key: str, visualization: bytes, ttl_seconds: int = 1800
    ) -> bool:
        """
        Cache visualization.
