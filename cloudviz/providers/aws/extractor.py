...
class AWSResourceExtractor(BaseResourceExtractor):
    ...
    async def get_account_info(self) -> AWSSubscription:
        ...
        try:
            ...
            for region_data in regions_response["Regions"]:
                ...
                region_ec2 = self.session.client(
                    "ec2", region_name=region_data["RegionName"]
                )
                try:
                ...
                regions.append(
                    {
                        ...
                        "display_name": region_data["RegionName"]
                        .replace("-", " ")
                        .title(),
            ...
            return AWSSubscription(
                account_id=self._account_id, organization_id=org_id, regions=regions
            )
    ...
    async def extract_ec2_instances(self, region: str) -> List[AWSEC2Instance]:
        ...
        try:
                ...
                for instance in reservation["Instances"]:
                    ...
                    instance_resource = AWSEC2Instance(
                        ...
                        name=self._get_tag_value(
                            instance.get("Tags", []), "Name", instance["InstanceId"]
                        ),
            ...
            logger.info(
                "Extracted %d EC2 instances from region %s", len(instances), region
            )
            return instances

        except ClientError as e:
            logger.error(
                "Failed to extract EC2 instances from region %s: %s", region, str(e)
            )
            return []
        ...
        try:
            ...
            for bucket_data in response["Buckets"]:
                ...
                try:
                    ...
                    logger.warning(
                        "Failed to get details for S3 bucket %s: %s",
                        bucket_name,
                        str(e),
                    )
                    continue
    ...
    def _get_tag_value(
        self, tags: List[Dict[str, str]], key: str, default: str = ""
    ) -> str:
    ...
    def _extract_tags(self, tags: List[Dict[str, str]]) -> Dict[str, str]:
        ...
        return {
            tag["Key"]: tag["Value"] for tag in tags if "Key" in tag and "Value" in tag
        }
