...
class GCPResourceExtractor(BaseResourceExtractor):
        ...
        try:
            ...
            try:
                request = compute_v1.AggregatedListInstancesRequest(
                    project=self.project_id, max_results=1
                )
    ...
    async def extract_compute_instances(
        ...
        try:
            ...
            if zone:
                ...
                for instance in response:
                    instance_resource = await self._convert_compute_instance(
                        instance, zone
                    )
    ...
    async def extract_resources_by_region(
        self, region: str, resource_types: Optional[List[str]] = None
    ) -> List[GCPResource]:
        """
    ...
    async def extract_global_resources(
    ...
    async def _convert_compute_instance(
        self, instance, zone: str
    ) -> GCPComputeInstance:
        """Convert GCP compute instance to CloudViz model."""
        # Extract machine type from URL
        machine_type = (
            instance.machine_type.split("/")[-1] if instance.machine_type else "unknown"
        )
        ...
        if instance.network_interfaces:
            ...
            subnetwork = (
                interface.subnetwork.split("/")[-1] if interface.subnetwork else None
            )
        ...
        if instance.disks:
            disk_size_gb = sum(
                disk.disk_size_gb for disk in instance.disks if disk.disk_size_gb
            )
        ...
        return GCPComputeInstance(
            ...
            image=(
                instance.source_machine_image.split("/")[-1]
                if instance.source_machine_image
                else "unknown"
            ),
            ...
            creation_timestamp=datetime.fromisoformat(
                instance.creation_timestamp.rstrip("Z")
            ),
            labels=dict(instance.labels) if instance.labels else {},
            tags=dict(instance.labels) if instance.labels else {},
            preemptible=(
                instance.scheduling.preemptible if instance.scheduling else False
            ),
