...
from cloudviz.core.utils import LoggerMixin, get_logger, retry_with_backoff
from cloudviz.providers.azure.models import (
    AzureResource,
    AzureResourceType,
    get_azure_resource_type_from_string,
)
...
class AzureResourceExtractor(ResourceExtractor, LoggerMixin):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.authentication_method = self.config.get(
    ...
    async def authenticate(self) -> bool:
        ...
        try:
            self.log_info(
                "Starting Azure authentication", method=self.authentication_method
            )
    ...
    async def _authenticate_service_principal(self) -> bool:
        ...
        if result.returncode == 0:
            ...
            self.log_error(
                "Service principal authentication failed", error=result.stderr
            )
            return False
    ...
    async def _authenticate_managed_identity(self) -> bool:
        ...
        if result.returncode == 0:
            ...
            self.log_error(
                "Managed identity authentication failed", error=result.stderr
            )
            return False
    ...
    async def _execute_powershell(self, script: str) -> subprocess.CompletedProcess:
        ...
        try:
            ...
            stdout, stderr = await asyncio.wait_for(
                process.communicate(), timeout=self.powershell_timeout
            )
    ...
    async def extract_resources(
        ...
        **kwargs,
        ...
        self.log_info(
            "Starting Azure resource extraction",
            scope=scope.value,
        ...
        inventory = ResourceInventory(
            ...
            metadata={
                ...
                "config": {
                    k: v for k, v in self.config.items() if k != "client_secret"
                },
            },
        )
        ...
        try:
            ...
            if scope == ExtractionScope.SUBSCRIPTION:
                resources = await self._extract_subscription_resources(
                    scope_identifier, resource_types, tags
                )
            elif scope == ExtractionScope.RESOURCE_GROUP:
                resources = await self._extract_resource_group_resources(
                    scope_identifier, resource_types, tags
                )
            elif scope == ExtractionScope.TAG:
                resources = await self._extract_tagged_resources(
                    scope_identifier, tags, resource_types
                )
            else:
            ...
            self.log_info(
                "Azure resource extraction completed",
            ...
            self.log_error(
                "Azure resource extraction failed", exc_info=True, error=str(e)
            )
            raise
    ...
    async def _extract_tagged_resources(
        ...
        script = self._build_extraction_script(
            scope="tag", identifier=tag_key, resource_types=resource_types, tags=tags
        )
...
"""
    ...
    def _parse_resources_from_output(self, output: str) -> List[AzureResource]:
        ...
        try:
            ...
            self.log_error(
                "Failed to parse resource JSON", error=str(e), output=output[:500]
            )
        except Exception as e:
            self.log_error(
                "Failed to process resource data", exc_info=True, error=str(e)
            )

        return resources

    def _create_azure_resource_from_data(
        ...
        try:
            ...
            self.log_error(
                "Failed to create Azure resource",
                exc_info=True,
                error=str(e),
                resource_data=data,
            )
    ...
    async def extract_resource_relationships(
        ...
        try:
            self.log_info(
                "Extracting Azure resource relationships", resource_count=len(resources)
            )
            ...
            relationships.extend(
                self._extract_network_relationships(resources, resource_lookup)
            )
            relationships.extend(
                self._extract_dependency_relationships(resources, resource_lookup)
            )
            relationships.extend(
                self._extract_management_relationships(resources, resource_lookup)
            )

            self.log_info(
                "Resource relationship extraction completed",
                relationship_count=len(relationships),
            )

        except Exception as e:
            self.log_error(
                "Failed to extract resource relationships", exc_info=True, error=str(e)
            )

        return relationships

    def _extract_containment_relationships(
        self, resources: List[AzureResource]
    ) -> List[ResourceRelationship]:
    ...
    def _extract_network_relationships(
        self, resources: List[AzureResource], resource_lookup: Dict[str, AzureResource]
    ) -> List[ResourceRelationship]:
        ...
        vnets = [
            r
            for r in resources
            if r.get_azure_resource_type() == AzureResourceType.VIRTUAL_NETWORK
        ]
        subnets = [
            r
            for r in resources
            if r.get_azure_resource_type() == AzureResourceType.SUBNET
        ]
        nics = [
            r
            for r in resources
            if r.get_azure_resource_type() == AzureResourceType.NETWORK_INTERFACE
        ]
        nsgs = [
            r
            for r in resources
            if r.get_azure_resource_type() == AzureResourceType.NETWORK_SECURITY_GROUP
        ]
        ...
        def extract_resource_ids(obj, path=""):
            if isinstance(obj, dict):
                for key, value in obj.items():
                    if (
                        key in ["id", "resourceId"]
                        and isinstance(value, str)
                        and "/subscriptions/" in value
                    ):
    ...
    async def get_available_scopes(self) -> Dict[str, List[str]]:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                scopes["resource_groups"] = [
                    rg["ResourceGroupName"] for rg in resource_groups
                ]

        except Exception as e:
            self.log_error(
                "Failed to get available scopes", exc_info=True, error=str(e)
            )

        return scopes

    async def validate_scope(
        self, scope: ExtractionScope, scope_identifier: str
    ) -> bool:
