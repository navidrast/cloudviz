...
from cloudviz.core.models import CloudProvider, CloudResource, ResourceType
...
class AzureResource(CloudResource):
    ...
    def get_cost_information(self) -> Dict[str, Any]:
        ...
        if self.tags:
            cost_info["cost_center"] = self.tags.get("CostCenter") or self.tags.get(
                "costCenter"
            )
            cost_info["project"] = self.tags.get("Project") or self.tags.get("project")
            cost_info["department"] = self.tags.get("Department") or self.tags.get(
                "department"
            )
    ...
    def get_compliance_information(self) -> Dict[str, Any]:
        ...
        if self.tags:
            compliance_info["environment"] = self.tags.get(
                "Environment"
