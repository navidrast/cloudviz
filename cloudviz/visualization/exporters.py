...
class DiagramExporter(LoggerMixin):
    ...
    async def export_diagram(
        ...
        **kwargs
        ...
        self.log_info("Diagram exported successfully",
                     format=format_type,
                     path=output_path,
        ...
        **kwargs
        ...
        metadata = {
            ...
            'inventory_info': {
                ...
                'scope': inventory.extraction_scope.value if inventory.extraction_scope else None,
    ...
    def _generate_resource_summary(self, inventory: ResourceInventory) -> Dict[str, Any]:
        ...
        for resource in inventory.resources:
            ...
            summary['by_resource_group'][rg] = summary['by_resource_group'].get(rg, 0) + 1
        ...
        **kwargs
        ...
        for format_type in formats:
            ...
            try:
                ...
                # Render diagram
                diagram_content = await engine.render(
                    inventory,
                    format_type,
                    **kwargs
                )
                ...
                self.log_error("Failed to export format",
                             format=format_type,
                             error=str(e),
                             exc_info=True)
        
        self.log_info("Multi-format export completed",
    ...
    async def _export_to_gcp_storage(
        ...
        **kwargs
        ...
        try:
            ...
            raise RuntimeError("GCP Storage export requires google-cloud-storage package")
        except Exception as e:
        ...
        for file_path in output_dir.iterdir():
            if file_path.is_file() and file_path.stat().st_mtime < cutoff_time:
                try:
                    ...
                    self.log_warning("Failed to delete old export",
                                   path=str(file_path),
                                   error=str(e))
