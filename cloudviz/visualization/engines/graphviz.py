...
class GraphvizEngine(VisualizationEngine, LoggerMixin):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.splines = self.config.get(
            "splines", "ortho"
        )  # ortho, spline, line, polyline
    ...
    async def render(
        ...
        **kwargs,
        ...
        if not await self.validate_options(output_format, theme, layout):
            raise ValueError(
                f"Invalid rendering options: format={output_format}, theme={theme}, layout={layout}"
            )
        ...
        try:
            ...
            dot_content = self._generate_dot_content(
                inventory, theme, layout, rankdir, splines, **kwargs
            )

            self.log_info(
                "Graphviz DOT diagram generated successfully",
            ...
            self.log_error(
                "Failed to render Graphviz DOT diagram", exc_info=True, error=str(e)
            )
            raise
    ...
    def _get_graph_attributes(
        self, rankdir: str, splines: str, layout: str, theme: str
    ) -> List[str]:
    ...
    def _add_resource_groups(
        self, resources: List[CloudResource], theme: str
    ) -> List[str]:
        ...
        for rg_name, rg_resources in groups.items():
            if rg_name and rg_resources:
                ...
                for resource in rg_resources:
                    node_def = self._create_resource_node(
                        resource, theme, indent="        "
                    )
    ...
    def _create_resource_node(
        self, resource: CloudResource, theme: str, indent: str = "    "
    ) -> str:
    ...
    def _get_resource_node_attributes(
        self, resource: CloudResource, theme: str
    ) -> List[str]:
    ...
    def _add_relationships(
        self, relationships: List[ResourceRelationship], theme: str
    ) -> List[str]:
    ...
    def _create_relationship_edge(
        self, relationship: ResourceRelationship, theme: str
    ) -> Optional[str]:
    ...
    def _get_relationship_color(
        self, relationship_type: RelationshipType, theme: str
    ) -> str:
    ...
    def _add_layout_constraints(
        self, layout: str, resources: List[CloudResource]
    ) -> List[str]:
        """Add layout-specific constraints."""
        lines = []
            ...
            if len(by_type) > 1:
                ...
                for res_type, type_resources in by_type.items():
                    if len(type_resources) > 1:
                        node_ids = [
                            f'"{self._sanitize_id(r.id)}"' for r in type_resources
                        ]
