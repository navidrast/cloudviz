...
class ImageEngine(VisualizationEngine, LoggerMixin):
    ...
    async def render(
        ...
        **kwargs,
        ...
        try:
            if source_format == "mermaid" or (
                source_format == "auto" and self._should_use_mermaid(layout)
            ):
                return await self._render_from_mermaid(
                    inventory, output_format, theme, layout, **kwargs
                )
            else:
                return await self._render_from_graphviz(
                    inventory, output_format, theme, layout, **kwargs
                )

        except Exception as e:
            self.log_error(
                "Failed to render image diagram", exc_info=True, error=str(e)
            )
            raise
    ...
    async def _render_from_mermaid(
        ...
        **kwargs,
        ...
        if not await self._check_mermaid_cli():
            raise RuntimeError(
                "Mermaid CLI (mmdc) not found. Please install @mermaid-js/mermaid-cli"
            )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if output_file.exists():
                    ...
                    self.log_info(
                        "Mermaid image rendered successfully", size=len(image_data)
                    )
                    return image_data
                else:
    ...
    async def _render_from_graphviz(
        ...
        **kwargs,
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Build command
            cmd = [tool, f"-T{output_format}", "-o", str(output_file), str(input_file)]
            ...
            try:
                ...
                if output_file.exists():
                    image_data = output_file.read_bytes()
                    self.log_info(
                        "Graphviz image rendered successfully", size=len(image_data)
                    )
                    return image_data
                else:
    ...
    async def _check_graphviz(self) -> bool:
        ...
        try:
            result = subprocess.run(
                [self.graphviz_path, "-V"], capture_output=True, text=True, timeout=10
            )
            return result.returncode == 0
        except (subprocess.TimeoutExpired, FileNotFoundError):
            return False
    ...
    async def render_from_source(
        ...
        **kwargs,
        ...
        if source_format.lower() in ["mermaid", "mmd"]:
            return await self._render_source_with_mermaid(
                source, output_format, **kwargs
            )
        elif source_format.lower() in ["dot", "gv", "graphviz"]:
            return await self._render_source_with_graphviz(
                source, output_format, **kwargs
            )
        else:
    ...
    async def _render_source_with_mermaid(
        self, source: bytes, output_format: str, **kwargs
    ) -> bytes:
        """Render image from Mermaid source."""
        if not await self._check_mermaid_cli():
