...
class MermaidEngine(VisualizationEngine, LoggerMixin):
        ...
        **kwargs,
        ...
        try:
            ...
            if layout == "hierarchical":
                diagram = self._generate_hierarchical_diagram(
                    inventory, theme, **kwargs
                )
            ...
            self.log_info(
                "Mermaid diagram generated successfully", diagram_length=len(diagram)
            )
            ...
            self.log_error(
                "Failed to render Mermaid diagram", exc_info=True, error=str(e)
            )
            raise

    def _generate_hierarchical_diagram(
        self, inventory: ResourceInventory, theme: str, **kwargs
    ) -> str:
        ...
        if self.group_by_resource_group:
            resource_groups = self._group_resources_by_resource_group(
                inventory.resources
            )
    ...
    def _generate_graph_diagram(
        ...
        for resource in inventory.resources:
            node_def = self._create_resource_node(
                resource, theme, indent="    ", style="graph"
            )
            lines.append(node_def)
    ...
    def _create_relationship_edge(
        self, relationship: ResourceRelationship, style: str = "flowchart"
    ) -> Optional[str]:
